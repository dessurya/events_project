...
class InterFaceController extends Controller
{
    ...
    private function dtableConfig()
    {
        return [
            ...
            'action' => [
                ["route" => "interface.form", "title" => "Update Interface", "action" => "update", "select" => true, "confirm" => false, "multiple" => false]
            ]
        ];
    }
    ...
    {
        return view('_pages.interface.form', ['config' => $this->formConfig()])->render();
    }
    ...
    public function list(Request $input)
    {
        ...
        return view('_pages.interface.index', compact('config'));
    }
    ...
    public function form(Request $input)
    {
        ...
        if ($input->id != "true") {
            ...
            if ($find->type == 'text') {
            	$content = view('_pages.interface.form_text',['find'=>$find,'config' => $this->formConfig()])->render();
            }else if($find->type == 'picture'){
            	$content = view('_pages.interface.form_picture',['find'=>$find,'config' => $this->formConfig()])->render();
            }else if($find->type == 'content'){
            	$content = view('_pages.interface.form_content',['find'=>$find,'config' => $this->formConfig()])->render();
            	$summernote = true;
